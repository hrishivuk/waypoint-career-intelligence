...
export function SavedJobDetail({ detail }: { detail: SavedJobAnalysisDetail }) {
  ...
  return (
    <article>
      ...
      <section aria-labelledby="decision-title" className="py-8"><h2 id="decision-title" className="text-xl font-semibold text-foreground">Decision</h2><p className="mt-2 text-sm text-muted-foreground">{recommendation.description}</p><div className="mt-5 grid gap-4 lg:grid-cols-2"><EvidenceList title="Why it could fit" items={analysis.strengths} icon={CheckCircle2} tone="success" empty="No specific strengths were stored." /><EvidenceList title="What needs attention" items={[...analysis.blockers, ...analysis.gaps, ...analysis.uncertainties]} icon={AlertTriangle} tone="warning" empty="No blockers or gaps were stored." /></div></section>
